...
export default function AdminWithdrawals() {
  ...
  const approve = async (id) => {
    const utrNumber = utrInputs[id];
    if (!utrNumber) return alert("Enter UTR Number");
      ...
      {filteredRequests.map((r) => (
        ...
        >
          <div className="grid md:grid-cols-3 gap-4 text-sm mb-6">
            ...
            <div>
              <p className="text-gray-400">Name</p>
              <p>{r.userId?.name || "N/A"}</p>
            </div>

            <div>
              <p className="text-gray-400">Account ID</p>
              <p className="text-yellow-400">
                {r.userId?.accountId || "N/A"}
              </p>
            </div>

            <div>
              <p className="text-gray-400">Mobile</p>
              <p>{r.userId?.phone || "N/A"}</p>
            </div>

            <div>
              <p className="text-gray-400">Telegram ID</p>
              <p>{r.userId?.telegramId || "N/A"}</p>
            </div>
            ...
            <div className="col-span-2">
              ...
              {r.paymentMethod === "UPI" ? (
                <p>UPI ID: {r.paymentDetails?.upiId || "N/A"}</p>
              ) : (
                <>
                  <p>Bank: {r.paymentDetails?.bankName || "N/A"}</p>
                  <p>Account No: {r.paymentDetails?.accountNumber || "N/A"}</p>
                  <p>IFSC: {r.paymentDetails?.ifsc || "N/A"}</p>
                </>
              )}
            </div>
